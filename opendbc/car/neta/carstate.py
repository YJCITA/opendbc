...
from opendbc.car.neta.values import DBC, CanBus, GearShifter, CarControllerParams,  NETA_CARS, CruiseButtons
...
class CarState(CarStateBase):
  ...
  def update_neta_s(self, pt_cp, cam_cp):
    ...
    self.steer_takeover = abs(ret.steeringTorque) > self.CCP.STEER_DRIVER_TAKEROVER
    ...
    ## brake pedal
    ret.brake = pt_cp.vl["IDB_Fr01_0E5"]["IDB1_BrakPedalDisp"] / 100.0  # TODO pt_cp.vl["EBB_2_A"]["EBB_BrkPedPst"] / 100.0  # FIXME: this is pressure in Bar, not sure what OP expects
    ...
    # ACC状态 沿用原车的状态机
    cruise_speed_raw = cam_cp.vl["ADCS_Fr08_193"]["ADCS8_longitudCtrlSetSpeed"] * CV.KPH_TO_MS
    if not self.enable_op_acc_state_manage:
      ...
      if self.curise_speed_set_cool_counter >= 30:
      ...
      if acc_distance_index_set_cur < 1:
        acc_distance_index_set_cur = 1
      if acc_distance_index_set_cur > 3:
        acc_distance_index_set_cur = 1
    ...
    ret.cruiseState.gearPos = gear_pos
  ...
  def get_can_parsers(CP, CP_SP):
    ...
    # return CANParser(DBC[CP.carFingerprint][Bus.pt], messages, CanBus(CP).ECAN)
    return {
  ...
  #   return CANParser(DBC[CP.carFingerprint][Bus.pt], messages, CanBus(CP).CAM)
